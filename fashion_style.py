...
with graph.as_default():
    ...
    # TODO : 2. Create weights and biases of decoder's *second* layer. (5%)
...
with tf.Session(graph=graph, config=config) as session:
    ...
    train_features_new = tf.sigmoid(tf.matmul((tf.matmul(train_features, encoder_w1) + encoder_b1), encoder_w2) + encoder_b2).eval()
    test_features_new = tf.sigmoid(tf.matmul((tf.matmul(test_features, encoder_w1) + encoder_b1), encoder_w2) + encoder_b2).eval()
